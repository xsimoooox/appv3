  ...
  return (
    <div className="w-full max-w-md mx-auto min-h-screen bg-[#f5f5f5] text-[#111111] px-4 pt-5 pb-[88px] select-none animate-fade-in">
      <header className="mb-5">
        ...
        <p className="text-[10px] text-[#5F5F72] font-semibold mt-1">Sessions et appels enregistrés</p>
      </header>

      {calls.length === 0 ? (
        <p className="text-center text-[11px] text-[#666680888] font-semibold py-12">
        ...
        <div className="space-y-2">
          {calls.map((call) => (
            <div key={call.id} className="rounded-[12px] border border-[#e5e5e5] bg-[#fafafa] p-3 flex items-center gap-3">
              <div className="w-9 h-9 rounded-[10px] bg-[#EAF5EB] text-[#2E7D32] flex items-center justify-center shrink-0">
                <PhoneIncoming size={18} strokeWidth={2.1} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-extrabold truncate">{call.name}</p>
                <p className="text-[9px] text-[#5F5F72] font-semibold mt-0.5">{call.when} — {call.duration}</p>
              </div>
              <span className="text-[8px] font-black uppercase text-[#2E7D32]">{call.status}</span>
            </div>
          ))}
        </div>
      )}
